      ...
      messages: [
        {
          role: "system",
          content:
            "You are a startup advisor. Given a struggling startup idea and its weakness, suggest 3 adjacent pivot ideas that address the core weakness while preserving founder strengths. Return JSON array.",
        },
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "user",
          content: `You are a startup advisor. Given a struggling startup idea and its weakness, suggest 3 adjacent pivot ideas that address the core weakness while preserving founder strengths. Return JSON array.\n\n${prompt}`,
        },
      ],
    }),
  });
...
export async function POST(req: NextRequest) {
  ...
  const { idea, readinessScore, weakness, payment } = body;

  if (!idea?.trim() || !weakness) {
  ...
  const prompt = `Startup idea: ${idea.trim()}
Current readiness score: ${readinessScore}/100
Biggest weakness: ${weakness}

Suggest 3 adjacent pivot ideas that address the ${weakness} weakness. Return ONLY a JSON array:
[
  {
    "idea": "pivot idea name and brief description",
    "whyBetter": "why this pivot addresses the weakness",
    "keyChange": "the single most important thing to change",
  ...
  try {
    ...
    console.error("Pivot advisor error:", err);
