...
export async function POST(req: NextRequest) {
  ...
  const { idea, businessModel, stage, payment } = body;

  if (!idea?.trim() || !businessModel || !stage?.trim()) {
  ...
  const prompt = `You are a startup metrics expert. Identify the perfect North Star Metric for this startup.

Startup idea: ${idea.trim()}
Business model: ${businessModel}
Current stage: ${stage.trim()}

Return ONLY valid JSON in this exact format:
{
  "metric": "exact name of the North Star Metric",
  "why": "2-3 sentences explaining why this is the right metric for this business model and stage",
  "howToMeasure": "specific instructions on how to track and measure this metric",
  "currentBenchmark": "what a good benchmark looks like at this stage (e.g., week-over-week growth %)",
  "warningSignals": ["warning signal 1", "warning signal 2", "warning signal 3"],
  "supportingMetrics": ["supporting metric 1", "supporting metric 2", "supporting metric 3", "supporting metric 4"]
}`;
  ...
  try {
    ...
    console.error("North star error:", err);
