...
const FEATURE = "interview_script";
const TIMEOUT = 30000;
...
export async function POST(req: NextRequest) {
  ...
  const { idea, targetCustomer, assumption, payment } = body;

  if (!idea?.trim() || !targetCustomer?.trim() || !assumption?.trim()) {
  ...
  const prompt = `You are an expert at customer discovery interviews for startups. Create a customer interview script.

Startup idea: ${idea.trim()}
Target customer: ${targetCustomer.trim()}
Biggest assumption to validate: ${assumption.trim()}

Return ONLY valid JSON in this exact format:
{
  "opening": "2-3 sentence opening to build rapport and set context",
  "questions": [
    {
  ...
  "closing": "closing statement and next steps"
}

Generate exactly 15 questions that validate the assumption. Focus on past behavior, not hypothetical. No leading questions.`;
  ...
  try {
    ...
    console.error("Interview script error:", err);
